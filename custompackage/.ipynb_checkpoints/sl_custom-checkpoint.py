...
class SparseLinear(nn.Module):
    ...
    def __init__(self, in_features, out_features, bias=True, sparsity=0.9, connectivity=None, small_world=False, dynamic=False, deltaT=6000, Tend=150000, alpha=0.1, max_size=1e8, sqgl_true=False, positive=False):
    ...
    def reset_parameters(self):
        if self.sqgl_true:
            torch.nn.init.uniform_(self.weights, a=0.99, b=1.01)
        else:
            torch.nn.init.normal_(self.weights, mean=0.0, std=math.sqrt(2/(self.in_features*(1 - self.sparsity))))
            if self.positive:
                self.weights.data = torch.abs(self.weights.data)
...
#     def reset_parameters(self):
#         torch.nn.init.normal_(self.weights, mean=0.0, std=math.sqrt(2/(self.in_features*(1 - self.sparsity)))
#         print('reset parameters', math.sqrt(2/(self.in_features*(1 - self.sparsity)))
#         bound = 1 / self.in_features**0.5
# #         nn.init.uniform_(self.weights, -bound, bound)
#         if self.bias is not None:
#             nn.init.uniform_(self.bias, -bound, bound)
